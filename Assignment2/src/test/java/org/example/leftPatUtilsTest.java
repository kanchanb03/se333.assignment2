...
class LeftPadUtilsTest {
    ...
    @Test
    public void testLeftPadBoundaries() {
        String original = "cat";
        String result = LeftPadUtils.leftPad(original, 3, "XYZ");
        assertEquals("cat", result, "When size == str.length, no padding");

        original = "cat";
        result = LeftPadUtils.leftPad(original, 7, "ABCD");
        assertEquals("ABCDcat", result, "Pads == padLen => padStr.concat(str)");

        original = "cat";
        result = LeftPadUtils.leftPad(original, 5, "ABCD");
        assertEquals("ABcat", result,"pads < padLen => substring(0,pads)+str");
    }
